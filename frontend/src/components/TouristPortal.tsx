...
export const TouristPortal: React.FC<TouristPortalProps> = ({
  ...
  const handleExecuteSosSend = (forceError = false) => {
    setSosStep('sending');
    setSosSendingProgress(15);
    setSosErrorMessage(null);

    setTimeout(() => {
      setSosSendingProgress(50);
    }, 600);
    ...
    setTimeout(() => {
      if (forceError) {
        setSosStep('error');
        setSosErrorMessage('Network signal drop detected in Solang valley sector. Local relay timeout.');
      } else {
        ...
        setIncidentRef(ref);
        setSosActive(true);
        onTriggerSos(authenticatedUser?.name || 'Elena Rostova', `${lat.toFixed(4)}, ${lng.toFixed(4)} (${activeGeoFenceZone.name})`);
      }
    }, 1800);
  };
