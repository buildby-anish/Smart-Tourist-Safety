...
const LEVELS: Record<SafetyLevel, { label: string; body: string; color: string; bg: string; icon: typeof Shield }> = {
...
export default function SafetyBanner({ level = 'safe', onAlertsTap }: Props) {
  ...
  return (
    <div
      ...
      style={{ background: cfg.bg, border: `1px solid ${cfg.color}30`, backdropFilter: 'blur(12px)' }}
    >
      <Icon size={15} style={{ color: cfg.color, flexShrink: 0 }} strokeWidth={2.5} />
      <div className="flex-1 min-w-0">
        <span className="text-xs font-bold" style={{ color: cfg.color }}>{cfg.label}</span>
        <p className="text-[11px] leading-snug mt-0.5 line-clamp-1" style={{ color: cfg.color, opacity: 0.8 }}>{cfg.body}</p>
      </div>
      <button onClick={onAlertsTap} className="flex items-center gap-0.5 flex-shrink-0 transition-opacity hover:opacity-70" aria-label="View alerts">
        <span className="text-xs font-semibold" style={{ color: cfg.color }}>Details</span>
        <ChevronRight size={12} style={{ color: cfg.color }} />
      </button>
      <button onClick={() => setDismissed(true)} className="flex-shrink-0 transition-opacity hover:opacity-60" aria-label="Dismiss">
