...
## 0. Integration Pass — Status Update (implemented)

The connections below were implemented against the actual backend and frontend source (not just this document). **The per-row "Backend Connection Status" columns in §2 onward reflect the state *before* this pass and are now stale for the rows listed here** — they're left as-is as a historical record rather than rewritten row-by-row, per the minimal-change instruction. Treat this section as authoritative for current status.

| Feature | Now connected to | Notes |
| :--- | :--- | :--- |
| Gateway MFA login | `POST /auth/register` (fallback) → `POST /authority/login` | Badge ID → `username`, Auth Code → `password` (already a masked input, no UI change). No seeded authority accounts exist, so a first-time badge auto-registers, then logs in — preserves the "demo creds just work" UX while making every login a real backend session. |
| Tourist sign-up | `POST /auth/register` → `POST /auth/login` → `PATCH /tourists/{id}` | The sign-up form has no password field, so credentials are derived deterministically from the phone number (see `deriveTouristCredentials` in `lib/api.ts`). This is a pragmatic bridge, not production-grade auth — flagged for the user to replace with real credential collection if this ships. |
| Tourist sign-in | `POST /auth/login` (via `loginTouristByPhone`) | Re-derives the same credentials from the entered phone. Only succeeds if that phone registered through this app in the current backend session; otherwise falls back unchanged to the existing local demo lookup (graceful degrade, matches the app's existing offline-tolerant design). |
| Logout (tourist & authority) | `POST /auth/logout` | Fire-and-forget; local session is always cleared regardless of the backend call's outcome. |
| SOS trigger | `POST /sos` | Unchanged endpoint (was already wired), but now sends the *real* backend `tourist_id` obtained at sign-up/sign-in instead of a cosmetic mock ID like `TR-88219` — SOS submissions against a real tourist record now actually succeed instead of always failing tourist lookup. |
| "Cancel SOS & Mark Safe" (tourist side) | `PATCH /incidents/{incident_id}` (`status: RESOLVED`) | Uses the `incident_id` returned by the `POST /sos` call. |
| "Mark Safe" (authority → tourist tracking) | `PATCH /incidents/{incident_id}` (`status: RESOLVED`) | Resolves that tourist's most recent open backend-linked incident, if any. |
| "Dispatch Unit" | `PATCH /incidents/{incident_id}` (`status: RESPONDING`) | |
| "Resolve Case" | `PATCH /incidents/{incident_id}` (`status: RESOLVED`) | |
| Authority incident board | `GET /authority/incidents`, `GET /authority/tourists/{id}`, `GET /authority/incidents/{id}/location` | Real backend incidents (created via tourist SOS) are fetched on authority login and merged into the existing local incident list, enriching each with tourist name/phone and resolved lat/lng where available. |
| Tourist interception / KYC lookup | `GET /authority/tourists/{id}` | Only triggered when the locally-found tourist carries a real `tourist_id` (i.e. registered through this app against the backend); otherwise the existing local-only record is shown unchanged. |
| Broadcast → Alerts bridge | `POST /alerts` | **Known mismatch, not a full connection:** `alerts` in the DB is a per-incident notification log (`incident_id` + `channel` + single `recipient`), not a region-wide broadcast campaign — there is no backend concept matching "publish an SMS blast to a geofenced region." Publishing a broadcast still works exactly as before (local UI/history unchanged) and additionally logs a real `SMS` alert row against every currently active backend-linked incident, as the closest faithful use of the existing schema without inventing new backend behavior. |

### Explicitly left unconnected (flagged, not silently mocked)
- **Itinerary** (`itinerary_entries` table) — no backend router exists; already flagged in `codebase.md` §9 as a net-new feature requiring a stop-and-confirm before building, per the integration task's own rules. Left as local-only state.
- **Location consent Grant/Decline** — `DATABASE.md`'s `tourists` table has no consent column; nothing to connect to without inventing a DB field. Left as local-only state.
- **Digital pass "Download PDF"** — client-side only; no backend PDF-generation endpoint exists. Left as-is.
- **AI Safety Chatbot** (`ModuleAIHub`) — no backend AI/chat endpoint exists in the documented API surface. Left as-is.
- **Analytics/Audit CSV export** — no backend endpoint for audit log export exists. Left as-is (audit logs themselves are still recorded locally exactly as before).

### New/changed files
- `frontend/src/lib/api.ts` — extended with auth, tourist, incident, location, alert, and authority API functions (see `codebase.md` §7 for the full list) plus orchestration helpers (`authenticateAuthority`, `registerAndLoginTourist`, `loginTouristByPhone`, `deriveTouristCredentials`).
- `frontend/src/App.tsx`, `frontend/src/components/Gateway.tsx`, `frontend/src/components/TouristPortal.tsx`, `frontend/src/components/ModuleTouristTracking.tsx` — wired to the above.
- `frontend/src/types.ts` — added optional `backendIncidentId?: string` to `SOSIncident`, bridging the app's existing mock incident IDs (e.g. `SOS-9021`) with real backend UUIDs so PATCH calls target the right record.
- `backend/.env` (new, local dev only) — sets `CORS_ALLOWED_ORIGINS` for `localhost:5173`/`localhost:3000`, since the default is empty and blocks all browser requests; nothing else in `config.py`/CORS logic was changed.
